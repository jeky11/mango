	...
	: IRabbitMqConsumer
{
	...
	public async Task StartAsync(CancellationToken cancellationToken)
	{
		...
		await RegisterQueueHandlerAsync(
			_connection, _topicAndQueueNames.OrderCreatedTopic, _topicAndQueueNames.OrderCreatedRewardsSubscription,
	...
	private async Task OnOrderPlacedRequestReceived(IChannel channel, BasicDeliverEventArgs arg) =>
		await HandleMessageAsync<RewardsMessage, IRewardService>(
			channel, arg, (service, rewardsMessage) => service.UpdateRewardsAsync(rewardsMessage));
